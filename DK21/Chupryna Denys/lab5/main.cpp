...
void display()
{
    ...
    glBegin(GL_POLYGON); //верхня основа
    glColor3f(0.0f, 1.0f, 0.0f);
    glVertex3f(-0.5f, 1.0f, 0.80f);
  ...
  glBegin(GL_POLYGON);//нижня основа
    ...
    glBegin(GL_QUADS); //грані

    //червона
    glColor3f(1.0f, 0.0f, 0.0f);
    glVertex3f(0.5f, 1.0f, 0.8f);
    glVertex3f(-0.5f, 1.0f, 0.8f);
    glVertex3f(-0.5, -1.0f, 0.8f);
    glVertex3f(0.5, -1.0f, 0.8f);
    //жовта
...
void KeyEvents(unsigned char key, int x, int y)//Функція обробки події, що виникає при натисканні клавіш на клавіатурі
{
    switch (key)
	{
	case 'a': //обертання відносно осі X проти годинникової стрілки
		a_x += xdelta;
		break;
	case 'd': //обертання відносно осі X за годинниковою стрілкою
		a_x -= xdelta;
		break;
	}
}
