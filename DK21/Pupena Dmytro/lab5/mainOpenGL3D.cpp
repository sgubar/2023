...
// Функція обробки події, що виникає при використанні колеса
void mouse(int button, int state, int x, int y)
{
    // Якщо вертимо колесиком миші вперед то зображення наближується, назад, зменшується
    if (button == 3) {
