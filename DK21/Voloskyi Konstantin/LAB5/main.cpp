...
#include <GL/glut.h>
#include <math.h>
#endif PI 3.14159265358979323846
...
void display()
{
    ...
    // Визначення вершин основи правильного трикутника
    GLfloat vertices[3][2] = {
        {-0.5f, -0.5f * sqrt(3.0f) / 2.0f},  // Ліва вершина
        {0.5f, -0.5f * sqrt(3.0f) / 2.0f},   // Права вершина
        {0.0f, 0.5f * sqrt(3.0f) / 2.0f}     // Верхня вершина
    };

    // Визначення висоти та масштабу призми
    GLfloat height = 2.0f;
    GLfloat scale = 0.5f;

    // Відображення основи призми
    glBegin(GL_TRIANGLES);
    glColor3f(1.0f, 0.0f, 0.0f);  // Червоний колір
    glVertex2f(vertices[0][0], vertices[0][1]);  // Ліва вершина
    glVertex2f(vertices[1][0], vertices[1][1]);  // Права вершина
    glVertex2f(vertices[2][0], vertices[2][1]);  // Верхня вершина
    glEnd();

    // Відображення бічних сторін призми
    glBegin(GL_QUADS);
   

    // Бічна сторона 1
    glColor3f(0.0f, 0.0f, 1.0f);  
    glVertex3f(vertices[0][0], vertices[0][1], 0.0f);              // Ліва нижня вершина
    glVertex3f(vertices[1][0], vertices[1][1], 0.0f);              // Права нижня вершина
    glVertex3f(vertices[1][0], vertices[1][1], height * scale);    // Права верхня вершина
    glVertex3f(vertices[0][0], vertices[0][1], height * scale);    // Ліва верхня вершина

    // Бічна сторона 2
    glColor3f(0.0f, 1.0f, 1.0f);  
    glVertex3f(vertices[1][0], vertices[1][1], 0.0f);              // Права нижня вершина
    glVertex3f(vertices[2][0], vertices[2][1], 0.0f);              // Права верхня вершина
    glVertex3f(vertices[2][0], vertices[2][1], height * scale);    // Верхня вершина
    glVertex3f(vertices[1][0], vertices[1][1], height * scale);    // Права верхня вершина
    ...
    // Відображення дна призми
    glBegin(GL_TRIANGLES);
    glColor3f(1.0f, 1.0f, 1.0f);  
    glVertex3f(vertices[0][0], vertices[0][1], height * scale);    // Ліва вершина верхньої основи
    glVertex3f(vertices[1][0], vertices[1][1], height * scale);    // Права вершина верхньої основи
    glVertex3f(vertices[2][0], vertices[2][1], height * scale);    // Верхня вершина верхньої основи
    glEnd();
